...
from forwarding.tracking.tracking_deepsort.deep_sort_app import track_single_sequence_deepsort
...
class TrackingForwarder(RecurrentDetectionForwarder):
  def __init__(self, engine):
    ...
    detection_confidence_threshold_car = self.config.float("detection_confidence_threshold_car", 0.55) # mark
    detection_confidence_threshold_pedestrian = self.config.float("detection_confidence_threshold_pedestrian", 0.95) # mark
    ...
    association_threshold_car = self.config.float("association_threshold_car", 0.2)     # for cosine
    association_threshold_pedestrian = self.config.float("association_threshold_pedestrian", 0.4) # for cosine
    ...
    # add lambda and iou threshold
    lambda_car = self.config.float("lambda_car", 0.0)
    lambda_pedestrain = self.config.float("lambda_pedestrain", 0.0)
    max_iou_distance_car = self.config.float("max_iou_distance_car", 0.7)
    max_iou_distance_pedestrain = self.config.float("max_iou_distance_pedestrain", 0.7)
    self.tracker_options = {"tracker": tracker, "reid_comp": tracker_reid_comp,
                            ...
                            "lambda_pedestrain": lambda_pedestrain,
                            "max_iou_distance_car": max_iou_distance_car,
                            "max_iou_distance_pedestrain": max_iou_distance_pedestrain}
  ...
  def _forward_video(self, n_timesteps, tag):
    ...
    if self.do_tracking:
      if self.tracker_options["mask_iou_weight_car"] > 0.0 or \
        ...
        optical_flow = None
      hyp_tracks = track_single_sequence_deepsort(self.tracker_options, det_boxes, det_scores, reid_features, det_classes,
